...
- 用户确认 Chrome 位于本机，并选择另开独立调试窗口重新登录。使用独立浏览器目录，CDP 仅监听 127.0.0.1:9222；用户完成 X 登录后，真实 Python 观察器从推荐、关注、探索三个页面读取到 16 条内容。
- 用户先提供长期兴趣，再提供已有认识。通过 MCP request 原样传递，服务自己的 personal-context.json 形成 long_term_interest 和 existing_knowledge 两类事实；日志不记录原文。
- 第一次完整请求约 65 秒后返回 incomplete/judgement_execution。对模型适配器做临时、隔离诊断，仅输出 HTTP 状态、耗时和封闭结果类别；第一次诊断 requestId 不符合 pf:32位十六进制格式，纠正后继续。单候选判断能正常拒绝，但完整跟踪证明另一次 HTTP 200 响应体在 30003ms 触发 TimeoutError，确认 30 秒单次模型上限不足。
- 诊断期间调试窗口被切换至个人主页，观察器返回 source_window/incomplete。确认该页不在现有观察器入口范围，将独立试验窗口切回 X 首页后继续；不修改观察器接受范围。
- 14:00 左右：通过安装器将本机模型超时改为 90000ms，并重新启动本次独立服务；真实请求随后在 120007ms 碰到总请求上限。已完成的候选判断被持久化，未将超时伪装成业务空集。
- 随后备份本机私有配置，以原子写入设置 PERSONAL_FEED_TOOL_TIMEOUT_MS=300000，保留模型 90000ms；只停止、启动本次 personal-feed.service，配置权限仍为 0600。该总超时为本机配置，不是安装器默认值，未来重新 apply 需检查是否保留。
- 14:05 左右：再次通过真实 MCP SDK 调用运行中的用户服务，104849ms 后返回 one_link。此前已完成候选判断由 7 条推进至 11 条，说明调用在持续筛选。推荐 URL 不写入仓库日志。
- 结论：浏览器读取、个人语境、真实模型判断、MCP 返回单链接全链路已完成一次本机验收。推荐内容是否符合用户期待仍由用户判断；这不是质量评估结论。
- 本机当前参数：MCP 127.0.0.1:43180；CDP 127.0.0.1:9222；deepseek-v4-flash；单次模型超时 90 秒；工具总超时 300 秒。运行包来自 86be27e，后续提交仅补记验收文档。
