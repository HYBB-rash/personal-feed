...
- `incomplete`：按工具返回的阶段说明未完成；`source_window` 带 `reason` 时分别说明获取来源失败、仅完成部分来源观察或来源正文不足，不向用户照抄枚举。没有 `reason` 时只说明来源观察未完成，不猜测原因；不把它说成“没有内容”。其中 `personal_context` 带问题/token 时说明原 Feed 正在等待补充信息，呈现具体问题；没有问题时不自行编造问题或等待状态。
...
- `observe_context` 或 `process_feedback` 返回 `incomplete` 且仍带问题/token 时，明确说明“这次回答尚未处理成功”，将问题标为上次保留的问题，继续保留关联；不要把旧问题当作对本次回答新生成的追问，也不声称本次信息已经处理成功。
- 关联无法使用时，按工具返回的未完成说明本次无法对应回答；不重建旧关联或自动重发旧 Feed。MCP 错误不解释成空结果或已完成。

应用支持个人信息问答和原 Feed 自动续做；只消费工具实际返回的结果，不允许自行补出字段或伪造续做结果。`observe_context` 收到合法但当前无法关联的 token 时会返回未完成。
